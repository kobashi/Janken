...
1. 「はじめる」をクリックします。
2. 4秒間、上段の「本命」と下段の「フェイク」を何度でもクリックできます。ボタン位置は固定です。
3. 本命ボタンで実際に出す手を決めます。CPUは最後に押された手を3/4の確率で読み、その手に勝つ手を出します。
4. 本命を決めたあと別のフェイクを押すと、実際の手を変えずにCPUへ偽の手を見せられます。
5. 掛け声、手札登場、「判定中」の演出を経て勝敗が表示されます。
6. 「もう一回」で続けます。勝・敗・引き分け数は画面上部に残ります。
