...
グー・チョキ・パーを押すたび、対応する日本語の擬音を波形合成で発音します。CPUは選択時間中の「ジャン」「ケン」「ポン」それぞれでランダムな手を1件ずつ選び、合計3件を履歴にします。勝負画面ではCPUの最終手を除く2件を基準に、プレイヤー側の直近2件も同じ2拍で表示・発音します。両者は各拍で同時に切り替わります。最終手は履歴に含めず、判定演出後に対応発音とともに公開されます。
