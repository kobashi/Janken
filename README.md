...
開発中に登場した基礎概念と用語は、[じゃんけんゲームで学ぶ開発・プログラミング基礎](docs/programming-guide.md)にまとめています。
...
| v1.5.0 | [プレイする](https://kobashi.github.io/Janken/versions/v1.5.0/) |
| v1.4.0 | [プレイする](https://kobashi.github.io/Janken/versions/v1.4.0/) |
...
1. 「達人（0.5秒）」「侍・普通（1.0秒）」「若武者（1.5秒）」から後出しの難易度を選び、「はじめる」をクリックします。
2. 固定表示されたグー・チョキ・パーを何度でもクリックできます。CPUが「ポン」で最後の手を発音した後も、選んだ難易度に応じて0.5〜1.5秒間だけ後出しできます。
...
4.5〜5.5秒の制限時間内に一度も押さなかった場合は未選択となり、プレイヤーは自動的に敗北します。

グー・チョキ・パーを押すたび、対応する日本語の擬音を波形合成で発音します。CPUは選択時間中の「ジャン」「ケン」「ポン」それぞれでランダムな手を1件ずつ選び、合計3件を履歴にします。勝負画面ではプレイヤーの最終手を除く全履歴を、一定の再生時間へ均等割りして順番に再生します。CPUの過去2件は最初と最後の拍へ配置され、その2拍だけ両者の表示・発音が同期します。最終手は履歴に含めず、判定演出後に対応発音とともに公開されます。

拍子音は「ジャン・ケン・ポン」共通の短いクリック系です。手の発音には、グー＝低いドの矩形波ブザー、チョキ＝ソのノコギリ波、パー＝高いドのサイン波を割り当てています。チョキはピッチが下がってから上がる輪郭を持ち、全音にエンベロープを適用しています。Unity版・WebGL版とも10音の同時再生を想定した構成です。
